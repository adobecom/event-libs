...
[`swan-notifications-unc.js`](../event-libs/v1/features/swan-notifications/swan-notifications-unc.js)'s
`applyStage()`.
...
A page opts in with the `swan-notifications` metadata flag, whose value selects which
implementation runs — no authoring app, no per-event sheet:

- `feds` — this repo's local widget (`notification-widget.js`), mounted into federal's
  `.feds-notifications-wrapper` placeholder. Also requires the page to carry
  `gnav-notifications` metadata set to `on`; federal only renders that placeholder when
  it's present, and `notification-widget.js`'s `mountNotificationWidget()` skips its mount
  attempt entirely (no timeout wait) when it isn't.
- `unc` — this doc's UNC-engine integration (`swan-notifications-unc.js`, `unc-client.js`),
  described above. Depends on `universal-nav` metadata enabling the notifications component
  (see "Verifying the chain end-to-end" below) — unrelated to `gnav-notifications`, which
  only gates feds mode's placeholder.
- `off`, missing, or any other value — nothing runs.

Everything else is either derived from data already available (`eventName` from the
`tier-1-event-config` metadata already parsed elsewhere in this repo) or a hardcoded default
(5-minute reminder lead time; a 3-day `localNotificationPersistTillDays` re-show window,
sized for a multi-day conference; icon currently hardcoded to an empty string — no real
...
2. Add `<meta name="swan-notifications" content="unc">` to a test page that already
