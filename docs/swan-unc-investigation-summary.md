...
`unc-client.js` chain wires together correctly against real page metadata and DOM (this
investigation predates the feds/unc mode split — that middle hop is now
`swan-notifications-unc.js`, reached via `swan-notifications.js`'s mode router) — but
