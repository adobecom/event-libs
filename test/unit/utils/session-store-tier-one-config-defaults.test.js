...
describe('session-store: RF defaults when tier-1-event-config omits them', () => {
  ...
  it('falls back to STAGE_RF_API_URL/DEFAULT_RF_PROFILE_ID/the hardcoded register default/null eventStartMs/eventEndMs', () => {
    const eventApiConfig = getEventApiConfig();
    expect(eventApiConfig.apiUrl).to.equal(STAGE_RF_API_URL);
    expect(eventApiConfig.rfProfileId).to.equal(DEFAULT_RF_PROFILE_ID);
    expect(eventApiConfig.registerUrl).to.equal('/register');
    expect(eventApiConfig.eventStartMs).to.equal(null);
    expect(eventApiConfig.eventEndMs).to.equal(null);
  });
});
