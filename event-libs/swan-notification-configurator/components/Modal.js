...
// Relies on the generic (unprefixed) .modal-* styles from
// session-guide-configurator/components/Modal.css — tier-1-event-configurator.html
// already links session-guide-configurator.css unconditionally (it mounts that app as
// its own tab), and this app is never mounted anywhere else, so those classes are
// always present on the page by the time this renders. No separate Modal.css needed here.
  ...
  const renderActions = () => {
    ...
    return html`
      <div class="modal-actions">
        <button type="button" class="snc-btn snc-btn--outline snc-btn--l" onClick=${onClose}>
          ${cancelText}
        </button>
        <button type="button" class="snc-btn snc-btn--primary snc-btn--l" onClick=${onConfirm} disabled=${confirmDisabled}>
          ${confirmText}
        </button>
      </div>
    `;
    ...
    <div class="modal-overlay" onClick=${handleBackdropClick} role="dialog" aria-modal="true" aria-labelledby="modal-title">
      <div class="modal ${sizeClass}" ref=${modalRef} tabindex="-1" onKeyDown=${handleKeyDown} role="document">
        <div class="modal-header">
          ...
          <button type="button" class="snc-btn snc-btn--icon modal-close" onClick=${onClose} aria-label="Close modal">
