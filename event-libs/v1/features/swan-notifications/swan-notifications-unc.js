// Unc-mode orchestration, ported from feat/swan-notifications (PR #276). session-store.js
// never calls this directly — see swan-notifications.js, the mode router that dispatches
// here only when getSwanMode() === 'unc'. reconcileSwanNotifications() runs on every
...
//
// The router passes a third `isScheduleKnown` argument (feds mode's orphan-cleanup gate);
// this mode has no equivalent guard today (ported as-is from feat/swan-notifications) and
// simply ignores it.
