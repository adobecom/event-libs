import { buildDateTabs } from './DateTabs.js';
import { buildViewDropdown } from './ViewDropdown.js';

export function buildDrawerHeader(preact, store) {
  const { html, useState, useEffect, useRef } = preact;
  const { useSessionGuide } = store;
  const DateTabs = buildDateTabs(preact, store);
  const ViewDropdown = buildViewDropdown(preact, store);

  return function DrawerHeader({ onClose, onFilterToggle, filterOpen, hideClose }) {
