      ...
      <div class="sg-time-row__track">
        ...
        <div class="sg-time-row__viewport" ref=${viewportRef}>
          <div class="sg-time-row__cards" ref=${stripRef} style=${'transform:translateX(-' + tx + 'px)'}>
            ${sessions.map((s) => html`<div class=${'sg-time-row__card-wrap' + (dismissingIds.has(s.id) ? ' sg-time-row__card-wrap--collapsing' : '')} key=${s.id}><${SessionCard} session=${s} forceOnDemand=${forceOnDemand} /></div>`)}
          </div>
        </div>
