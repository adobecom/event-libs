...
export function getCaasLocaleKey() {
  const locale = getMetadata('locale') || getEventConfig()?.miloConfig?.locale?.ietf || 'en-US';
...
export function resolveTagWithGroup(tagId, tagsData, localeKey = 'en') {
...
export function resolveTagObjects(tagIdList, tagsData, localeKey = 'en') {
  if (!tagIdList) return [];
  return tagIdList
    .split(',')
...
function normalizeSessions(rawSessions, locationMap, registeredSessionIds, venueId, tagsData) {
  const localeKey = getCaasLocaleKey();
  const mapped = rawSessions.map((session) => {
    ...
    return {
      ...
      tags: resolveTagObjects(session.tags, tagsData, localeKey),
      sessionTimes,
      speakers,
