...
Light (the default) or dark. Pure CSS, no JS involved: `event-card.css` styles
`.section.dark .event-card .card-title/.card-description/.card-cta` directly off the
ancestor `.section`'s own `dark` class — the same plain class `decorate.js`'s
...
themes itself with no per-card/per-block wiring at all, and no dependency on `init()`
having run yet. A `dark-card` class present on the card itself is honored too, as a
manual override for a card that needs to force dark independent of its section —
`event-card.css` matches `.event-card.dark-card` the same way.
