...
Light (the default) or dark. Primarily section-driven: `init()`'s `getTheme()` checks
`el.closest('.section')` for a `dark` class — the same plain class `decorate.js`'s
`applyAreaTheme()` (and DA's own Section Metadata `style: dark` authoring) already
lands directly on the section, and that every other themed block in this repo keys
off. Put any card — hand-authored or generated by a caller like
`event-libs/v1/c2/blocks/featured-sessions/` — inside a dark-styled section and it
themes itself with no per-card/per-block wiring at all. A `dark-card` class present on
the card itself *before* `init()` runs is honored too, as a manual override for a card
that needs to force dark independent of its section. Either way, `getTheme()` sets
`data-card-theme="dark"` on the card (nothing for light, the implicit default), which
`event-card.css`'s `[data-card-theme="dark"]` rules key off to invert title/description/
CTA colors.
