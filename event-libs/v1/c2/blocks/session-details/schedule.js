/*
 * Add to schedule (MWPW-200288) — Pre-Live primary CTA of session-details.
 * Mirrors favorite.js: resolves the real session (for rfCode), toggles via the
 * shared schedule action (auth/registration gate + conflict modal + toast fire
 * automatically), and paints from the `scheduled` signal.
 */
...
// Figma "add to schedule" glyph (calendar + plus). fill: currentColor so it
// reads white on the dark CTA pill.
...
export function renderSchedule() {
  ...
  // Schedule keys on rfCode; resolve the real session (upgrade on catalog load).
