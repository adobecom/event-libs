...
**Stack:** Preact · Preact Signals (shared session state) · ES Modules · Milo (YouTube/adobetv embeds)

> **Status (last updated during Phase 0):** Phase 0 done — shared foundation utilities built and tested. An earlier version of this plan extracted a `SessionDetailPanel` component out of sessions-guide's `SessionDetailOverlay`; that was reverted once we confirmed Broadcast should instead trigger the *real*, already-mounted Session Guide widget via the existing `openSessionGuideDetail(sessionId)` mechanism (see Architecture decisions). No changes to sessions-guide's own code are needed for this story. The `?session=` URL param collision that surfaced from that decision is also resolved (Broadcast's entry param renamed to `?watch=`) — no PM decision needed after all.
...
| YouTube player | Use Milo's `libs/blocks/youtube/youtube.js` `LiteYTEmbed` directly (dynamic import), not event-libs' `event-youtube.js` | Avoids perpetuating the known YouTube-wrapper duplication (`event-youtube.js` carries live-chat logic Broadcast doesn't need) |
...
### Phase 1 — Core page shell, State 1, YouTube only
- `BroadcastApp.js` (root: mounts on `initSessionState()`, derives page state 1/2/3, owns layout)
- `PlayerHost.js` + `players/YouTubePlayerAdapter.js` only (MPC/MobileRider adapters come later)
- `AlsoLiveCarousel.js` / `UpNextCarousel.js` wrapping sessions-guide's `Carousel.js` + `LiveCard.js`; card click (whole card body, excluding CTAs) calls `openSessionGuideDetail(session.id)` — no local modal. Also Live header includes a "View all sessions" button alongside the persistent FAB
- `SessionInfoPanel.js` (collapsed/expanded); "Learn More" also calls `openSessionGuideDetail(session.id)`. Add-to-Schedule disabled (Favorite-only) once a session is live
- Full URL behavior for the single-player case: entry `?watch=`, strip-on-load, stale-session toast fallback, in-page switch via state-only `pushState`
- Session Guide FAB placement confirmed/wired

**Exit criteria**: on a DA draft page with `event-id`/`tier-1-event-config` metadata and real or `?serverTime=`-simulated live YouTube sessions, a user can load the page, see the correct default session autoplay, switch between concurrent YouTube sessions, open the session detail modal, and reload a stale/shared URL correctly — all without a page reload.
...
**YouTube adapter**: dynamically import `${miloLibs}/blocks/youtube/youtube.js` for `customElements.define('lite-youtube', ...)` and its static `loadYouTubeAPI()` helper. Broadcast must always use the `YT.Player` API path (not just Apple/mobile) — construct `new window.YT.Player(el, { videoId, playerVars: { autoplay: 1, playsinline: 1 } })` directly for `onStateChange` events and clean teardown.
