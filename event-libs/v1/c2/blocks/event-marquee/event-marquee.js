...
async function getSectionConfig(el, miloLibs) {
  ...
  if (metadata['countdown-end-time']) {
    const raw = metadata['countdown-end-time'].content[0]?.textContent.trim() || '';
    const parsed = Date.parse(raw);
    if (raw && Number.isFinite(parsed) && HAS_TZ_OFFSET.test(raw)) {
      config.countdownEndTime = parsed;
    } else if (raw) {
      window.lana?.log(`[event-marquee] invalid countdown-end-time (must include a UTC/offset designator, e.g. a trailing "Z"): ${raw}`);
    }
  }
...
function buildCountdown(targetMs) {
  const wrapper = createTag('div', { class: 'event-marquee-countdown' });
  const initial = formatCountdown(targetMs, getNowMs());
  createTag('p', { class: 'event-marquee-countdown-label' }, 'Session starts in:', { parent: wrapper });
...
export default async function init(el) {
  ...
  if (config.countdownEndTime != null) {
    const countdown = buildCountdown(config.countdownEndTime);
