...
function disconnectReparentObserverAndRemoveOrphans(chronoBoxEl) {
  ...
  roots.forEach((node) => {
    try {
      ...
      logError('chrono-box-c2,reparent-cleanup', 'reparent cleanup failed', error);
    }
  });
  roots.clear();
}
...
export function cleanupChronoBoxOutboundNodes(chronoBoxEl) {
  ...
  try {
    ...
    logError('chrono-box-c2,before-swap', 'before-swap event dispatch failed', error);
  }
  ...
  if (Array.isArray(fns)) {
    fns.forEach((fn) => {
      try {
        ...
        logError('chrono-box-c2,outbound-cleanup', 'outbound cleanup callback failed', error);
      }
    });
    chronoBoxEl._chronoBoxOutboundCleanup = [];
  }
...
function getSchedule(scheduleId) {
  ...
  try {
    ...
    logError('chrono-box-c2,schedule-parse', 'error parsing schedule', error);
  }

  if (!thisSchedule) {
    logWarning('chrono-box-c2,schedule-lookup', 'schedule not found', scheduleId);
    return null;
  }
...
async function createWorker(workerUrl) {
  ...
  try {
    ...
    logError('chrono-box-c2,worker-create', 'failed to create worker', error);
...
async function setScheduleToScheduleWorker(schedule, plugins, tabId) {
  ...
  try {
    ...
    logError('chrono-box-c2,worker-message', 'error posting message to worker', error);
    if (blobUrl) {
  ...
  worker.addEventListener('error', (event) => {
    logError('chrono-box-c2,worker-error', 'worker error', event.message || 'unknown');
  });
...
export default async function init(el) {
    ...
    if (cols.length >= 2) {
      ...
      if (key === 'schedule-id') {
        ...
        try {
          ...
          logError('chrono-box-c2,static-schedule-parse', 'error parsing static schedule', error);
        }
      }
    }
  });
  ...
  try {
    ...
    logError('chrono-box-c2,worker-init', 'error creating worker', error);
      ...
      loadFragment(a)
        .then(async () => {
          ...
          try {
            ...
            logError('chrono-box-c2,modal-hash', 'error opening modal from hash', error);
          }
          resolve();
        })
        .catch((error) => {
          logError('chrono-box-c2,fragment-load', `error loading fragment ${fragmentPath}`, error);
          el.removeAttribute('style');
