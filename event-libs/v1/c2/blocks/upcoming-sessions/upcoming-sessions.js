...
/**
 * Mirrors sessions-guide's CategoryBadge.js markup/classes/behavior exactly (size="sm"
 * only — the only size this card ever uses): looks up `track` via the shared
 * getTrackIcon() (tier-1-event-config.js — authored per-event via track-icon-config
 * metadata, falling back to a curated built-in default set), falling back again to
 * the "mainstage" entry if the track has no config at all, so a badge always renders.
 * The label is the raw track string itself, not a separate curated label.
 *
 * Icon resolution (resolveIcon) is async — it fetches an SVG sprite. The badge itself
 * is returned synchronously with an empty icon slot, and the icon is appended once
 * resolved, matching the same "badge appears immediately, icon a beat later" UX
 * sessions-guide's own Preact Icon.js already has (icon resolves in its own effect).
 */
