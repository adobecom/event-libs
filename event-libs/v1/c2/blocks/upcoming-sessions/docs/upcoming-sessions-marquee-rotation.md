...
### 14. Schedule-conflict modal — disabled for this event (double-booking allowed)

**Design update:** an earlier iteration of this section enabled the same
shared schedule-conflict modal Session Guide uses (`EVENT_CONFIG.
showConflictModal: true`), reusing `session-actions.js`/`action-feedback.js`/
`conflict-modal.js` as-is. That's since been reverted — **this event
explicitly allows double-booking**, so cards in this component must not
prompt a conflict modal at all.

- `EVENT_CONFIG.showConflictModal` (in `upcoming-sessions.js`) is `false`.
- With it `false`, `scheduleAction()` (`services/sessions/session-actions.js`)
  never calls `findScheduleConflict()` for this block's Add to Schedule
  clicks — scheduling always proceeds immediately, even against an
  overlapping already-scheduled session.
- This is a one-line config flip, not a code path removal — the shared
  conflict-detection/modal machinery (`session-actions.js`,
  `action-feedback.js`, `features/conflict-modal/conflict-modal.js`) is
  untouched and still fully functional for Session Guide and any other
  caller that opts in with `showConflictModal: true`. Only this block's own
  `EVENT_CONFIG` opts out.
- Auth/registration gating (`assertAuthorized()`) is unaffected — a
  logged-out or unregistered user still sees the login/registration toast
  before Add to Schedule can succeed, regardless of the conflict setting.

#### Test plan — no conflict modal, double-booking allowed

No automated tests yet (standing "no tests until dev complete" instruction
applies here too). Manual steps:

1. **Schedule two overlapping sessions.** Using the console-seeding recipe
   from the previous version of this test plan (inject a fake
   already-scheduled session into `session-store.js`'s shared `sessions`/
   `scheduled` signals with a time window overlapping one of this block's
   own cards — see git history for the exact snippet if needed), confirm
   clicking Add to Schedule on the overlapping card succeeds **immediately**
   — no modal appears, the card's schedule icon flips to scheduled, and both
   the fake session and the real one remain in `scheduled` simultaneously.
2. **Auth gating still applies.** Clear `localStorage.getItem('sg:dev-auth')`
   (or otherwise force `isLoggedIn`/`isRegistered` to not both be `true`)
   and confirm Add to Schedule still shows the login/registration toast —
   this check runs regardless of `showConflictModal`.
3. **Session Guide's own conflict modal is unaffected.** Separately confirm
   (in Session Guide, not this block) that scheduling two overlapping
   sessions there still shows its own conflict modal as before — this
   change only opts *this block* out via its own `EVENT_CONFIG`, it does
   not touch the shared conflict-detection logic itself.
...
| Schedule-conflict modal | `EVENT_CONFIG.showConflictModal: false` — Add to Schedule never prompts a conflict modal (§14) | This event allows double-booking; the shared conflict-detection/modal machinery itself is untouched, only this block's own config opts out |
