...
const HYDRATOR_CONFIG = {
  // Blocks that can be hydrated
  blocks: ['image-links'],
...
function extractTierFromClassList(classList, metadataKey) {
  for (const tier of HYDRATOR_CONFIG.filterConfig[metadataKey].tierKeywords) {
...
function hydrateImageLinksBlock(block) {
  let metadataKey = null;

  for (const [className, metaKey] of Object.entries(HYDRATOR_CONFIG.metadataMapping)) {
  ...
  const filterConf = HYDRATOR_CONFIG.filterConfig[metadataKey];
...
export function hydrateBlocks(area = document) {
  HYDRATOR_CONFIG.blocks.forEach((blockName) => {
    const blocks = area.querySelectorAll(`.${blockName}`);

    blocks.forEach((block) => {
      switch (blockName) {
        case 'image-links':
          hydrateImageLinksBlock(block);
          break;
        default:
          break;
      }
