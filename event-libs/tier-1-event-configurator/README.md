...
**`config.rfApiUrl`/`config.rfProfileId`, added for [MWPW-200311](https://jira.corp.adobe.com/browse/MWPW-200311).** Nested inside `config` like any other authored field (`trackIcons`, `allowDoubleBooking`) — deliberately *not* row-level, since the whole point is consolidating the RainFocus endpoint/profile id into the same single JSON payload an author already pastes into `tier-1-event-config` metadata, rather than requiring two more separate metadata rows. `event-libs/v1/utils/session-store.js` parses `tier-1-event-config` metadata and reads `rfApiUrl`/`rfProfileId` straight off it (falling back to legacy flat `rainfocus-api-url`/`rainfocus-api-profile-id` metadata, then to hardcoded defaults, for pages authored without the Configurator). Never carried over on Duplicate (`startDuplicateConfig` always resets both blank) — unlike a style setting such as `trackIcons`, silently reusing another event's RainFocus profile id would misroute this new event's live schedule/favorites calls at the wrong RF profile. Leaving `rfApiUrl` blank is fine: it falls back to `DEFAULT_RF_API_URL` (`https://www.adobe.com/max-api/`, confirmed live against real MAX 2025 traffic) in `event-libs/v1/services/sessions/rainfocus.js`. Leaving `rfProfileId` blank also falls back to a hardcoded default (`DEFAULT_RF_PROFILE_ID`, same file, currently `RF_PROFILE_IDS.max26`) so calls don't silently send `profileId: undefined` — update `DEFAULT_RF_PROFILE_ID` to point at the new year's entry in `RF_PROFILE_IDS` once the next MAX supersedes it.
